...
const App: React.FC = () => {
  ...
  const checkForUpdates = async () => {
    ...
    try {
      ...
      // Sync until the end of the NEXT month to allow early confirmation and prevent 429 Rate Limits
  ...
  const handleEmusysSync = async () => {
    try {
      ...
      const d = new Date();
      const firstDay = new Date(d.getFullYear(), d.getMonth(), 1).toISOString().split('T')[0] + ' 00:00:00';
      // Sync until the end of the NEXT month to allow early confirmation and prevent 429 Rate Limits from Emusys API
      const lastDay = new Date(d.getFullYear(), d.getMonth() + 2, 0).toISOString().split('T')[0] + ' 23:59:59';
