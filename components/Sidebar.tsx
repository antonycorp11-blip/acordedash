...
const Sidebar: React.FC<Props> = ({ currentView, setView, darkMode, setDarkMode, onImport, onSync, onReset, hasUpdates }) => {
  ...
  return (
    <aside className="w-full md:w-24 md:h-screen fixed bottom-0 left-0 md:relative flex md:flex-col items-center justify-center md:justify-start md:py-8 md:gap-10 bg-[#121212]/95 backdrop-blur-md z-[200] shadow-[0_-10px_30px_rgba(0,0,0,0.5)] md:shadow-2xl shrink-0 px-2 md:px-0 pb-[max(12px,env(safe-area-inset-bottom))] pt-2 md:pt-8 min-h-[85px] md:min-h-0 border-t border-white/5 md:border-t-0">
      {/* Desktop Logo */}
      <button
      ...
      <nav className="flex md:flex-col gap-3 md:gap-6 items-center flex-1 w-full justify-center md:justify-start overflow-x-auto no-scrollbar py-2 px-4 h-full">
        <button
          onClick={() => setView('dashboard')}
        ...
        <button
          className={`shrink-0 w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center transition-all ${currentView === 'teachers' ? activeClass : inactiveClass}`}
