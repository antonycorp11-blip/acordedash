...
const Sidebar: React.FC<Props> = ({ currentView, setView, darkMode, setDarkMode, onImport, onSync, onCloudSync, onCloudRestore, onReset, hasUpdates }) => {
  ...
  return (
    <aside className="w-full h-20 md:w-24 md:h-screen fixed bottom-0 md:relative flex md:flex-col items-center justify-center md:justify-start md:py-8 md:gap-10 bg-[#121212] z-[100] shadow-2xl shrink-0 overflow-x-hidden md:overflow-x-auto no-scrollbar px-2 md:px-0">
      {/* Desktop Logo */}
      <button
      ...
      <nav className="flex md:flex-col gap-0 md:gap-6 items-center flex-1 w-full justify-between md:justify-start">
        <button
          onClick={() => setView('dashboard')}
          className={`w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center transition-all ${currentView === 'dashboard' ? activeClass : inactiveClass}`}
          title="Início"
        >
        ...
        <button
          ...
          className={`w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center transition-all ${currentView === 'calendar' ? activeClass : inactiveClass}`}
          title="Calendário"
        >
        ...
        <button
          className={`hidden md:flex w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl items-center justify-center transition-all ${currentView === 'weekly' ? activeClass : inactiveClass}`}
        ...
        <button
          ...
          className={`w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center transition-all ${currentView === 'financial' ? activeClass : inactiveClass}`}
          title="Financeiro"
        >
        ...
        <button
          className={`hidden md:flex w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl items-center justify-center transition-all ${currentView === 'teachers' ? activeClass : inactiveClass}`}
        ...
        {/* Sync Buttons on Mobile */}
        <button
          onClick={onCloudSync}
          className="md:hidden w-12 h-12 rounded-xl flex items-center justify-center transition-all opacity-40 hover:opacity-100"
          title="Salvar Nuvem"
        >
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M3 15a4 4 0 004 4h9a5 5 0 10-.1-9.999 5.002 5.002 0 10-9.78 2.096A4.001 4.001 0 003 15z" /></svg>
        </button>
        <button
        ...
        <button
          onClick={onSync}
          className="md:hidden w-12 h-12 rounded-xl flex items-center justify-center transition-all opacity-40 hover:opacity-100 relative"
        ...
        >
          ...
          <div className={`absolute top-2 right-2 w-2 h-2 rounded-full border border-[#121212] ${hasUpdates ? 'bg-red-500 animate-pulse' : 'bg-green-500'}`}></div>
        </button>
      </nav>
